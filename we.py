...
class WordEmbedding:
    def __init__(self, fname, fname2=None, src="w2v", corpus_path = None):
        if src == "w2v" or src == "fasttext":
            self.thresh = None
            self.max_words = None
            self.desc = fname
            ...
            #                 v /= np.linalg.norm(v)
                ...
                assert(fname.endswith(".txt") and fname2.endswith(".txt"))
                i2w = dict()
                # words = []
                count = 0
                count_weird = 0
                with open(fname, "r", encoding='utf8') as f_w2i:
                    for line in f_w2i:
                        line_ok = re.match(r"^.* [0-9]+\n$", line)
                        if line_ok:
                            s = line.split()
                            if (len(s) == 2):
                                i2w[int(s[1])] = s[0]
                                ...
                                count_weird += 1
                                i2w[int(s[0])] = "SOMEWEIRDWORDONESHOULDNEVERUSE"+str(count_weird)
                                print("Got weird line: "+line)
                        else:
                            print(line)
                            raise ValueError("Wrong line format")
                        count += 1
                words = [i2w[i] for i in range(0,len(i2w))]
                print("count=%d, len(words)=%d, len(i2w)=%d" % (count, len(words), len(i2w)))
                vecs = np.empty((len(i2w), 300))
                with open(fname2, "r", encoding='utf8') as f_i2v:
                    for line in f_i2v:
                        line_ok = re.match(r"^[0-9]+( -?0\.[0-9]+){300}\n$", line)
                        if line_ok:
                            s = line.split()
                            vec = np.array([float(x) for x in s[1:]])
                            idx = int(s[0])
                            vecs[idx] = vec
                            if (idx % 10000) == 0:
                                print(idx)
    ...
    def reindex(self):
        ...
        #DIRTY WORKAROUND IF REPETITION:
        # self.index = dict()
        # for i in range(0, len(self.words)):
        #     if self.words[i] in self.index.keys():
        #         print(str(i) + ": ", self.words[i])
        #         new_w = self.words[i] + "SOMETHINGWEIRD"
        #         self.index[new_w] = i
        #     else:
        #         self.index[self.words[i]] = i
...
def drop(u, v, debias_scale=0):
    # if should be redundant TODO: check
    if debias_scale == 0:
        return u - v * u.dot(v) / v.dot(v)
    else:
        return scale_projection(u,v,debias_scale)

def scale_projection(u, v, c=0):
    return u + (c-1)*v * u.dot(v) / v.dot(v)